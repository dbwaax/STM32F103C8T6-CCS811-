...
u8 Single_ReadIIC(u8 Slave_Address,u8 REG_Address,u8 *REG_data,u8 length)
{
	...
	sendIICByte(Slave_Address);    //发送设备地址+写信号
 	if(waitAck()==NACK)
	{
		stopIIC(); 
		return 0;
	...
	sendIICByte(REG_Address);     //发送存储单元地址
 	if(waitAck()==NACK)
	{
		stopIIC(); 
		return 0;
	...
	sendIICByte(Slave_Address+1);  //发送设备地址+读信号
 	if(waitAck()==NACK)
	{
		stopIIC(); 
		return 0;
	...
	while(length-1)
	{
		*REG_data++=receiveIICByte(ACK);       //读出寄存器数据
		length--;
	}
	*REG_data=receiveIICByte(NACK);  
